...
def start_programm():
    ...
    if 'error' in lovefinder_data:
        sys.exit(f"Что-то пошло не так :( {lovefinder_data['error']}")
    if 'reason' in lovefinder_data:
    ...
    if finded_users:
        ...
        # id всех найденных пользователей
        finded_users_id = result_data_find.keys()
        result_data_in_db = get_users(lovefinder_data, age_range[0], age_range[1], finded_users_id)
        ...
        set_result_data_find = set(finded_users_id)
        set_result_data_in_db = set(result_data_in_db)
        set_result_data_find -= set_result_data_in_db
        data_for_insert = list()
        for value in set_result_data_find:
            data_for_insert.append(result_data_find[value][0])
        ...
        # Вставим данные в БД
        insert_users(lovefinder_data, temp, data_for_insert)
